...
void free_stmt(Node_Statement stmt) {
	switch (stmt.statement_type) {
		...
		case var_assignment_type:
			switch (stmt.statement_value.var_assignment.destination.destination_type) {
				case assgn_dest_var_name_type:
					break;

				case assgn_dest_deref_type:
					free_expresion(*stmt.statement_value.var_assignment.destination.destination_value.deref_expresion);
					break;

				case assgn_dest_subscript_type:
					free_expresion(*stmt.statement_value.var_assignment.destination.destination_value.index_expr);
					break;
			}
